...
export default function EastCoastMap({
    ...
    const [selectedPort, setSelectedPort] = useState(propActivePort || flowDest || "Paradip");
    const [selectedOrigin, setSelectedOrigin] = useState(propOriginPort || flowOrigin || "Newcastle");
    ...
    // Sync external activePort & originPort props or flow requirement if changed
    useEffect(() => {
        const nextDest = propActivePort || flowDest;
        if (nextDest) setSelectedPort(nextDest);
    }, [propActivePort, flowDest]);

    useEffect(() => {
        const nextOrigin = propOriginPort || flowOrigin;
        if (nextOrigin) setSelectedOrigin(nextOrigin);
    }, [propOriginPort, flowOrigin]);
            ...
            <div className="bg-slate-900 text-white px-5 py-3.5 flex flex-wrap items-center justify-between gap-3 border-b border-slate-800">
                ...
                <div className="flex items-center bg-slate-800 p-1 rounded-xl border border-slate-700 gap-1">
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg font-bold text-xs flex items-center gap-1.5 transition-all ${mapEngine === "leaflet"
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg font-bold text-xs flex items-center gap-1.5 transition-all ${mapEngine === "tactical"
                ...
                <div className="flex items-center gap-2 text-xs">
                    ...
                    <div className="flex items-center bg-slate-800 rounded border border-slate-700 p-0.5">
                        {[1, 2, 5].map((s) => (
                            <button
                                ...
                                className={`px-2 py-1 rounded text-[11px] font-mono font-bold ${simSpeed === s ? "bg-blue-600 text-white" : "text-slate-400 hover:text-white"
                                    }`}
                            >
                                {s}x
                            </button>
                    ...
                    <button
                        ...
                        className={`px-2.5 py-1.5 rounded border text-xs flex items-center gap-1 font-medium ${showShippingLanes ? "bg-blue-950/80 border-blue-500 text-blue-300" : "bg-slate-800 border-slate-700 text-slate-400"
                            }`}
                ...
                <div className="bg-slate-950 text-white px-5 py-3 border-b border-blue-900/60 flex flex-wrap items-center justify-between gap-4 font-mono text-xs shadow-inner">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-white font-bold transition-all shadow ${simProgress >= 100 ? "bg-emerald-700 cursor-default" : "bg-blue-600 hover:bg-blue-500"
                                }`}
                        >
                            {simProgress >= 100 ? (
                                <>
                        ...
                        <button
                            ...
                            className={`px-2.5 py-1.5 rounded-lg text-[11px] font-bold border transition-all ${weatherDelayActive
                        ...
                        <button
                            ...
                            className={`px-2.5 py-1.5 rounded-lg text-[11px] font-bold border transition-all ${portCongestionActive
            ...
            {mapEngine === "leaflet" ? (
                <div className="w-full h-[620px]">
                    <NauticalLeafletMap
                        ...
                        onResetScenarios={() => { if (flow.resetSimulation) flow.resetSimulation(); }}
                    />
                </div>
            ) : (
                <>
                            ...
                            {activePortObj && showRoute && (
                                <g>
                                    ...
                                    {showSimulation && (() => {
                                        ...
                                        const p0 = { x: 980, y: 600 };
                                        const p1 = { x: 820, y: activePortObj.y + 100 };
                                        ...
                                        return (
                                            <g transform={`translate(${bx}, ${by}) rotate(${bAngle})`}>
                                                ...
                                                <g transform={`rotate(${-bAngle}) translate(22, -12)`}>
                                                    <rect x="0" y="-10" width={vesselName.length * 6.5 + 16} height="20" rx="3" fill="#020617" stroke="#10B981" strokeWidth="1" />
                                                    <text x="6" y="3.5" fill="#6EE7B7" fontSize="9" fontFamily="JetBrains Mono" fontWeight="bold">
                                                        🚢 {vesselName} ({Math.round(simProgress)}%)
                                                    </text>
                                                </g>
                                            </g>
                                        );
                        ...
                        {hoveredPort && !selectedVessel && (
                            ...
                            >
                                <div className="flex items-center justify-between border-b border-slate-800 pb-1.5">
                                    ...
                                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${hoveredPort.congestion === "High" ? "bg-red-500/20 text-red-400 border border-red-500/40" :
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${activePortObj.congestion === "High" ? "bg-red-500/20 text-red-300 border-red-500/40" :
